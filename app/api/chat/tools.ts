...
export const setUserEmail = tool(
  ...
  },
  {
    name: "set_user_email",
    description:
      "Set the user email for the onboarding process. This tool automatically validates the email address and rejects invalid emails, free webmails and disposable emails. You can manually bypass the validation check if required.",
    schema: z.object({
      email: z.string().describe("Email address to validate."),
      validation_bypass: z
        .boolean()
        .optional()
        .describe("If true, will bypass the validation check."),
    }),
  }
);

export const saveUseCaseSummary = tool(
  async (input: { summary: string }, config: LangGraphRunnableConfig) => {
    const thread_id = config.configurable?.thread_id;
